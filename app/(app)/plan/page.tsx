...
import { PlanPageSkeleton } from "./_components/plan-page-skeleton";
import {
...
export default function PlanPage() {
    ...
    const capacityModalRef = useRef<SetCapacityModalHandle | null>(null);
        ...
        async (weekStart?: string, options?: { showLoading?: boolean }) => {
            ...
            try {
                ...
                if (!response.ok || !responseData) {
                    throw new Error(
                        responseData?.error ?? "Failed to load weekly plan.",
                    );
                }
            ...
            if (mutation.kind === "add") {
                ...
                setData((prev) => {
                    ...
                    const nextOccurrence: PlanOccurrence = {
                        ...
                        habit_has_micro:
                            responseData.occurrence.habit_has_micro,
    ...
    const remainingHabits = useMemo<RemainingHabit[]>(() => {
        ...
        data.days.forEach((day) => {
            day.occurrences.forEach((occurrence) => {
                const currentCount =
                    plannedCountByHabit.get(occurrence.habit_id) ?? 0;
        ...
        return data.habits
            .map((habit) => {
                ...
                const targetFrequency = Math.max(
                    0,
                    Math.round(habit.freq_per_week),
                );
    ...
    const overloadMoveSuggestion =
        useMemo<OverloadMoveSuggestion | null>(() => {
            if (!data || !isOverloaded) {
                return null;
            }
            ...
            data.days.forEach((day) => {
                day.occurrences.forEach((occurrence) => {
                    ...
                    if (
                        occurrence.planned_weight_cu >
                        biggestOccurrence.planned_weight_cu
                    ) {
                        biggestOccurrence = occurrence;
                    }
                });
            ...
            <section className="flex flex-col gap-4 w-full">
                        ...
                        <Dialog>
                            ...
                            <DialogContent>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Add habit occurrences across Mon-Sun,
                                        keep planned CU under weekly capacity,
                                        and use micro-steps or re-scheduling
                                        when overloaded.
                                    </DialogDescription>
                                </DialogHeader>
                            </DialogContent>
                ...
                <section className="rounded-xl border border-border bg-card p-4 text-card-foreground shadow-sm">
                    <div className="grid gap-4 xl:grid-cols-[1.25fr_1fr_auto]">
                        ...
                        <CapacityMeter
                            plannedCu={plannedCu}
                            capacityCu={weeklyCapacity}
                        />

                        <div className="flex flex-col gap-2 xl:w-42.5">
                            <Button
                                type="button"
                                onClick={() => capacityModalRef.current?.open()}
                            >
                                Set capacity
                            </Button>
                            <Button
