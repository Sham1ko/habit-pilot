          ...
          {remainingHabits.length === 0 ? (
            ...
            remainingHabits.map((habit) => {
              const availableDays = days.filter(
                (day) => !day.occurrences.some((item) => item.habit_id === habit.id),
              );
                ...
                >
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2">
                    {habit.has_micro ? (
                      ...
                      <span className="text-xs text-muted-foreground">Standard only</span>
                    )}
                        ...
                        {availableDays.length > 0 ? (
                          availableDays.map((day) => (
                            <DropdownMenuItem
                              ...
                              onSelect={() => onAddOccurrence(habit.id, day.date)}
                            >
                              {formatDayLabel(day.date)}
                            </DropdownMenuItem>
                          ))
                        ) : (
                          <DropdownMenuItem disabled>All days used</DropdownMenuItem>
                        )}
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
          ...
          <div className="mt-3 space-y-2">
            {overloadMicroSuggestion ? (
              <Button
                ...
                onClick={() => onApplyMicroSuggestion(overloadMicroSuggestion.occurrenceId)}
              >
