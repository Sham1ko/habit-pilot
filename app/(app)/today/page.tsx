...
export default function TodayPage() {
  ...
  const handleAction = async (occurrenceId: string, action: ActionPayload["action"]) => {
    if (!data) {
      return;
    }
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="w-full max-w-sm rounded-xl border border-border bg-card p-4 text-card-foreground shadow-sm">
          <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
            ...
            <span>
              {weeklyCapacity ? `${formatCu(usedCu)} / ${formatCu(weeklyCapacity)} CU` : "Set weekly capacity"}
            </span>
          </div>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      ...
                      variant="outline"
                      disabled={isPending || isCompleted}
                      onClick={() => handleAction(item.occurrence_id, "skipped")}
                    >
                      Skip
                    </Button>
                  </div>
